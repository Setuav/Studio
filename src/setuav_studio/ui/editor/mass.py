...
WB_EXTENSION_ID = "org.setuav.weight-balance"
EXTENSION_ID = WB_EXTENSION_ID
...
class MassPropertiesEditor(PropertyTableMixin, QWidget):
    ...
    def __init__(
        ...
        self.setObjectName("core.mass_properties_editor")
        self._api = api
    ...
    def _load_component(self, component: dict[str, Any]) -> None:
        ...
        try:
            ...
            own_definition = extensions.get(WB_EXTENSION_ID)
    ...
    def _on_field_changed(self, _value: float) -> None:
        ...
        def change() -> None:
            ...
            component.pop("mass_expression", None)
            parameters = component.get("parameters")
            if isinstance(parameters, dict) and "mass" in parameters:
                parameters["mass"] = mass_g

            # Write native Core fields directly on component
            component["local_cg_mm"] = cg
            component["inertia_kg_m2"] = inertia

            # Backward-compatibility mirror in extensions
            extensions = component.setdefault("extensions", {})
            if not isinstance(extensions, dict):
                extensions = {}
                component["extensions"] = extensions
            definition = extensions.setdefault(WB_EXTENSION_ID, {})
            if not isinstance(definition, dict):
                definition = {}
                extensions[WB_EXTENSION_ID] = definition
            definition.update(
                {
