...
# -------------------------------------------------------------------------
# Global OpenVSP Total Scaling (fract) Functions
# -------------------------------------------------------------------------

def scale_wing_total_span(
    profiles: list[dict[str, Any]],
    target_span: float,
    sweep_loc: float = 0.25,
    symmetric: bool = True,
    ...
    """OpenVSP-style total span proportional scaling (UpdateTotalSpan)."""
    sections = profiles_to_sections(profiles, sweep_loc)
    if not sections:
        return profiles

    curr_panel_span = sum(s["span"] for s in sections)
    target_panel_span = (target_span / 2.0 - y_offset) if symmetric else target_span
    target_panel_span = max(target_panel_span, 1.0)

    fract = target_panel_span / max(curr_panel_span, 1e-6)
    for sec in sections:
        sec["span"] = max(sec["span"] * fract, 1.0)

    return sections_to_profiles(sections, profiles[0], sweep_loc)


def scale_wing_total_area(
    profiles: list[dict[str, Any]],
    target_area: float,
    sweep_loc: float = 0.25,
    symmetric: bool = True,
    ...
    """OpenVSP-style total area proportional scaling (UpdateTotalArea)."""
    sections = profiles_to_sections(profiles, sweep_loc)
    if not sections:
        return profiles

    c_root0 = sections[0]["root_chord"]
    s_center = (2.0 * y_offset * c_root0) if symmetric else 0.0
    panel_areas = [0.5 * (s["root_chord"] + s["tip_chord"]) * s["span"] for s in sections]
    curr_area = (2.0 * sum(panel_areas) + s_center) if symmetric else sum(panel_areas)

    target_area = max(target_area, 1.0)
    fract = math.sqrt(target_area / max(curr_area, 1e-6))

    for sec in sections:
        sec["root_chord"] = max(sec["root_chord"] * fract, 1.0)
        sec["tip_chord"] = max(sec["tip_chord"] * fract, 1.0)
        sec["span"] = max(sec["span"] * fract, 1.0)

    return sections_to_profiles(sections, profiles[0], sweep_loc)


def scale_wing_total_chord(
    profiles: list[dict[str, Any]],
    target_root_chord: float,
    sweep_loc: float = 0.25,
) -> list[dict[str, Any]]:
    """OpenVSP-style chord scaling (UpdateTotalChord)."""
    sections = profiles_to_sections(profiles, sweep_loc)
    if not sections:
        return profiles

    curr_root_c = sections[0]["root_chord"]
    fract = target_root_chord / max(curr_root_c, 1e-6)
